# Calculadora UX - Professional 3D Edition

![Leonardo Firme](https://img.shields.io/badge/Leonardo_Firme-fff1f0?style=for-the-badge&logo=data:image/png;base64,&logoColor=white)
![React](https://img.shields.io/badge/React-19-61DAFB?style=for-the-badge&logo=react)
![Tailwind CSS](https://img.shields.io/npm/v/tailwindcss?style=for-the-badge&logo=tailwindcss&logoColor=white&color=06B6D4&label=tailwindcss)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite)

![Demonstração do Template](./template.png)

Uma interface de alta precisão desenvolvida com **React 19** e **Tailwind CSS v4 (@latest)**, focada em entregar uma experiência de usuário (UX) tátil através de design neumórfico 3D e profundidade de hardware.

## 🚀 Diferenciais Técnicos

- **UX 3D Neumórfica**: Implementação de sombras projetadas e internas (`inset`) para simular volume físico e feedback de pressão mecânica nas teclas.
- **Persistent Formula History**: Lógica avançada no hook para manter o rastro da operação visível no visor mesmo após a exibição do resultado.
- **Tailwind v4 Clean Engine**: Utilização exclusiva de classes utilitárias nativas e variáveis de tema modernas, sem hacks de colchetes ou plugins legados.
- **Arquitetura Escalável**: Componentização atômica seguindo padrões de ERPs profissionais.

## 🛠️ Stack Utilizada

- **Core**: React 19 (Hooks, Transitions)
- **Estilização**: Tailwind CSS v4 (@latest)
- **Build Tool**: Vite
- **Tipografia**: Inter (Digital Standard)

## 📂 Estrutura do Projeto

```text
src/
├── components/
│   ├── display/
│   │   └── Screen.jsx      # Visor 3D com histórico persistente
│   ├── keyboard/
│   │   ├── Board.jsx       # Grid operacional 4x6
│   │   └── Key.jsx         # Teclas neumórficas com feedback ativo
│   └── layout/
│       └── CalculatorRoot.jsx # Chassi 3D com moldura chanfrada
├── hooks/
│   └── useCalculator.js    # Lógica de estado e motor aritmético
├── utils/
│   └── engine.js           # Processamento de expressões matemáticas
└── index.css               # Configuração de tema e variáveis v4

```

## ⚙️ Instalação e Execução

1. Clone o repositório:

```bash
git clone [https://github.com/LeonardoFirme/calculadora-ux.git](https://github.com/LeonardoFirme/calculadora-ux.git)

```

2. Instale as dependências:

```bash
npm install

```

3. Inicie o servidor de desenvolvimento:

```bash
npm run dev

```

## 🎨 Guia de Estilo (3D Spec)

| Elemento | Light Mode | Dark Mode |
| --- | --- | --- |
| **Chassi** | `bg-gray-50` / Shadow Neumorphic | `bg-gray-950` / Shadow Deep |
| **Visor** | `bg-linear-to-br` / Inset Shadow | `bg-linear-to-br` / Inset Shadow |
| **Teclas** | `text-gray-400` / `hover:bg-gray-50` | `text-gray-200` / `hover:bg-gray-900` |
| **Ação (=)** | `bg-gray-800` / `text-white` | `bg-gray-50` / `text-gray-950` |

---

Desenvolvido por **[Leonardo Firme](https://github.com/LeonardoFirme)**